a=[1,3,4,6,7,8,9,11,15,17,19,21,22,25,29,33,38,69,107]
def findd(start,end,list1,find_i):
    mid_value=int((start+end)/2)
    if len(list1[start:end])==1:
        if list1[start]!=find_i:
            print('没有找到')
            return
        else:
            print('找到了')
            return
        
    if list1[mid_value]>find_i:
        print(f'去找{list1[start]}到{list1[mid_value]}这个区间')
        findd(start,mid_value,list1,find_i)
    else:
        print(f'去找{list1[mid_value]}到{list1[end]}这个区间')
        findd(mid_value,end,list1,find_i)

    
findd(0,len(a)-1,a,8) 

# a = [1,3,4,6,7,8,9,11,15,17,19,21,22,25,29,33,38,69,107]


# def b_search(start,end,li,find_n):

#     mid_index = int( (start + end ) / 2 ) # 每次找到列表中间的位置的索引
#     if len(li[start:end]) == 1: # 碰头了，查完了
#         if li[start] != find_n: # 完了，找不到了
#             print("找不到该值.")
#             return
#         else:
#             print("找到了，",li[start])
#             return
#     if li[mid_index] > find_n: # the val you looking for should be in the left part
#         print("去左边一半的数据里查:",li[mid_index], li[start:mid_index])
#         b_search(start,mid_index,li,find_n)
#     else: # val in right
#         print("去右边一半的数据里查:",li[mid_index], li[mid_index:end])
#         b_search(mid_index,end,li,find_n)

# b_search(0,len(a)-1,a,8)
   